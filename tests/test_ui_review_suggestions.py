...
def conn(tmp_path: pathlib.Path) -> Iterator[sqlite3.Connection]:
    ...
    connection.execute(
        ...
        VALUES (7, 'company_name', '파이썬 소프트웨어 재단', '원문 하단 회사명이 다르다')
        """
    )
    try:
        yield connection
...
def test_제안이_있는_칸은_표와_모달에_제안_있음이_나온다(client: TestClient) -> None:
    ...
    assert 'hx-post="/ui/review/suggestions/3/company_name"' in modal
...
def test_수락하면_보정으로_들어가고_제안이_사라진다(
    ...
    response = client.post("/ui/review/suggestions/3/company_name", data={"action": "accept"})

    assert response.status_code == 200
    assert override_of(conn, "company_name") == "파이썬 소프트웨어 재단"
    assert suggestion_of(conn, "company_name") is None
...
def test_거절하면_제안만_사라지고_보정은_그대로다(
    ...
    response = client.post("/ui/review/suggestions/3/company_name", data={"action": "reject"})

    assert response.status_code == 200
    assert override_of(conn, "company_name") is None
    assert suggestion_of(conn, "company_name") is None
    assert override_count(conn) == 0
    assert "제안 있음" not in response.text
    assert "HX-Trigger-After-Settle" not in response.headers
...
def test_이미_처리된_제안을_다시_누르면_사유를_적고_닫지_않는다(
    ...
    client.post("/ui/review/suggestions/3/company_name", data={"action": "reject"})
...
def test_고칠_수_없는_필드나_모르는_처리는_사유를_적는다(client: TestClient) -> None:
    ...
    bad_action = client.post(
        "/ui/review/suggestions/3/company_name", data={"action": "delete"}
    ).text
