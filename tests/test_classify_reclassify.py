...
def store_parts(
    ...
    conn.executemany(
        ...
        " (raw_job_id, part, part_role, part_lines, model, responsibilities)"
...
async def test_한_번에_나눈_공고는_직무_목록을_알려주고_칸만_다시_채운다(
    ...
    assert rows(conn, SHORT, "part, responsibilities") == [
...
async def test_개수가_다르면_실패로_두고_기존_값을_그대로_둔다(conn: sqlite3.Connection) -> None:
    ...
    assert rows(conn, SHORT, "part, part_role, responsibilities") == [
        (1, "가", "옛 업무"),
        (2, "나", "옛 업무"),
        (3, "다", "옛 업무"),
    ]
