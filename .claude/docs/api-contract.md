...
**오공고 백엔드로는 이 API 가 아니라 밀어서 보낼 계획이다.** 오공고에 받는 엔드포인트가 이미
있어서 그쪽으로 맞추기로 했다 (`.claude/docs/spring-delivery.md`). 이 조회 API 는 그대로
둔다 — 다른 소비 측이 생길 수 있고, 무엇을 보냈는지 사람이 확인하는 자리이기도 하다.

전달 방향이 갈리면 `delivered_at` 의 뜻도 갈린다. 지금 이 컬럼은 아래 `POST /api/jobs/delivered`
로만 찍히고, 밀어서 보내는 경로가 생기면 그때 이 문서와 `.claude/rules/data-safety.md` 를
같은 커밋에서 고친다.
